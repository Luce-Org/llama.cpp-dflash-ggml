...
#endif // __clang__
template<int D, int ncols, ggml_type type_K, ggml_type type_V, bool use_logit_softcap, bool skip_masked> // D == head size
...
#pragma unroll
            for (int j = 0; j < ncols; ++j) {
                const bool q_in_bounds = ncols == 1 || ic0 + j < int(ne01.z);
                float mask_v = 0.0f;
                bool masked = false;
                if (mask && q_in_bounds) {
                    mask_v = slope*__half2float(maskh[j*ne11 + i_KQ]);
                    if constexpr (skip_masked) {
                        masked = mask_v <= -60000.0f;
                    }
                }

                float sum = masked ? 0.0f : vec_dot_KQ(K + i_KQ*nb11, Q_reg[j], Q_i32[j], Q_ds[j]);
...
#pragma unroll
            ...
            if constexpr (skip_masked) {
                KQ_reg[j] = KQ_masked[j] ? 0.0f : expf(KQ_reg[j] - KQ_max[j]);
            } else {
...
#pragma unroll
            for (int j = 0; j < ncols; ++j) {
                const half kq = KQ[j*nthreads + k];
                KQ_k[j] = __half2half2(kq);
                any_KQ_k = any_KQ_k || __half2float(kq) != 0.0f;
            }
            if constexpr (skip_masked) {
                if (!any_KQ_k) {
                    continue;
...
template <int D, int cols_per_block, ggml_type type_K, ggml_type type_V, bool use_logit_softcap, bool skip_masked>
void ggml_cuda_flash_attn_ext_vec_case_impl(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;

    const int nthreads = ggml_cuda_fattn_vec_get_nthreads_host(cc);
    const int nwarps   = nthreads / WARP_SIZE;
    fattn_kernel_t fattn_kernel = flash_attn_ext_vec<D, cols_per_block, type_K, type_V, use_logit_softcap, skip_masked>;
...
void ggml_cuda_flash_attn_ext_vec_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (Q->ne[1] == 1) {
        ...
        if (logit_softcap == 0.0f) {
            ...
            ggml_cuda_flash_attn_ext_vec_case_impl<D, cols_per_block, type_K, type_V, use_logit_softcap, skip_masked>(ctx, dst);
        } else {
...
template <int D, ggml_type type_K, ggml_type type_V>
void ggml_cuda_flash_attn_ext_vec_masked_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * KQV = dst;
    const ggml_tensor * Q   = dst->src[0];
    ...
    if (Q->ne[1] == 1) {
        ...
        if (logit_softcap == 0.0f) {
            constexpr bool use_logit_softcap = false;
            constexpr bool skip_masked = true;
            ggml_cuda_flash_attn_ext_vec_case_impl<D, cols_per_block, type_K, type_V, use_logit_softcap, skip_masked>(ctx, dst);
        } else {
            constexpr bool use_logit_softcap = true;
            constexpr bool skip_masked = true;
